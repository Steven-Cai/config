...
:END:
** About
This is Steven's Emacs configuration.

#+BEGIN_SRC emacs-lisp
;; The configuration works good on emacs27.1 on Ubuntu 20.04.
;; sudo apt-get install emacs27
(setq-default user-full-name "Steven Cai")
#+END_SRC
...
(use-package projectile :ensure
  ...
  (setq! projectile-completion-system 'ivy
         ...
         projectile-ignored-projects (list user-emacs-directory)
...
C-c p f  : Find files in the project.
...
(defun +ivy/switch-buffer ()
  ...
  (let ((buffer-list (buffer-list (selected-frame)))
        other-buffer)
    (while (and (null other-buffer) (setq other-buffer (pop buffer-list)))
      (when (or (minibufferp other-buffer) (get-buffer-window other-buffer))
        (setq other-buffer nil)))
...
;; (add-hook! 'before-save-hook #'delete-trailing-whitespace) ;; habit
...
*** Swiper
Press M-n(ivy-next-history-element) to get symbol-at-point into the minibuffer.
And/or press M-j(ivy-yank-work) to extend the minibuffer input with the next word.
#+BEGIN_SRC emacs-lisp
...
(use-package cc-mode
  ...
  (after! compile
    (add-to-list 'compilation-error-regexp-alist 'gcc)
    (add-to-list 'compilation-error-regexp-alist-alist
                 `(gcc
                   ,(rx
                     bol
                     (group-n 1 (: (* num) (not space) (*? nonl)))
                     (: ":" (* " ") (group-n 2 (+ num)))
                     (? ":" (* " ") (group-n 4 (+ num)))
                     ":" (* " ")
                     (| (group-n 6 (| "warning"))
                        (group-n 7 (| "info" "information" "informational" "instantiated from" "required from" "note"))
                        (| "error" "sorry,"))
                     (| " " ":"))
                   1 (2 . 3) (4 . 5) (6 . 7)))))
#+END_SRC
...
*** C#
#+BEGIN_SRC emacs-lisp
;; If you met tar not found error, trying 'package-refresh-contents' may solve the problem.
...
(use-package matlab-shell
  :init
  (setq! magic-mode-alist (assq-delete-all 'matlab-is-matlab-file magic-mode-alist))

  :config
  (setq! matlab-shell-command-switches '("-nodesktop" "-nosplash")
         matlab-shell-use-emacs-toolbox nil
         magic-mode-alist (assq-delete-all 'matlab-is-matlab-file magic-mode-alist))
...
(defun +org*expand-tags-fix (orig-fun match &optional single-as-list downcased)
  (funcall orig-fun (if downcased (downcase match) match) single-as-list downcased))
(advice-add #'org-tags-expand :around #'+org*expand-tags-fix)
#+END_SRC
...
(use-package ivy-bibtex :ensure
  ...
  (defun +bibtex*ivy-bibtex (orig-fun &optional arg local-lib)
    (let* ((local-bib (and (not (and (numberp arg) (= arg 0)))
      ...
      (funcall orig-fun arg local-bib)))
  (advice-add #'ivy-bibtex :around #'+bibtex*ivy-bibtex)
...
governed by the variable `bibtex-completion-display-formats'."
    (let* ((format (or (assoc-string (bibtex-completion-get-value "=type=" entry)
      ...
      (s-format
       ...
       (lambda (field)
         (let* ((field (split-string field ":"))
           ...
           (when (string= field-name "title")
             (let ((searchtitle (bibtex-completion-get-value "searchtitle" entry))
                   (maintitle (bibtex-completion-get-value "maintitle" entry))
                   (volume (bibtex-completion-get-value "volume" entry)))
               (setq field-value
                     (or searchtitle
                         (concat maintitle
                                 (when (and maintitle field-value) ": ")
                                 field-value
                                 (when (and (or maintitle field-value) volume) ", ")
                                 (when volume (concat "Vol. " volume)))))))
...
Return DEFAULT if FIELD is not present in ENTRY."
  ...
  (setq! bibtex-completion-bibliography +bibtex-bibliography
         ...
         bibtex-completion-additional-search-fields '("searchtitle" "maintitle" "volume" "keywords" "edition")
...
"
  ...
  (defun +ivy-bibtex-insert-org-link (candidate)
    (let ((key (cdr (assoc "=key=" (cdr candidate))))
          (title (bibtex-completion-apa-get-value "title" (cdr candidate))))
      (insert (format "[[bibtex:%s][%s]]" key title))))

  (ivy-add-actions 'ivy-bibtex '(("O" +ivy-bibtex-insert-org-link "Insert Org link"))))
...
**** ivy-bibtex integration in org-mode
#+begin_src emacs-lisp
(after! org
...
(use-package gdb-mi :quelpa (gdb-mi :fetcher git
  ...
  ;; Advice the function to do additional work in setting up windows.
  (defadvice gdb-setup-windows (after activate)
    ...
    (let* ((top-left (selected-window))
      ...
      (gdb--set-window-buffer bottom-left  (gdb--inferior-io-get-buffer session))
...
(use-package doom-themes-ext-treemacs
  ...
  :init (setq! doom-themes-treemacs-enable-variable-pitch nil))
#+END_SRC
...
*** xcscope
#+BEGIN_SRC emacs-lisp
(use-package xcscope :quelpa (xcscope :url "https://raw.githubusercontent.com/dkogan/xcscope.el/master/xcscope.el"
                                      :fetcher url)
  :init
  (require 'xcscope)
  (cscope-setup)
